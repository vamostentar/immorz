...
const configSchema = z.object({
  ...
  EMAIL_FROM: z.string().default('noreply@ribeirazul.pt'),
  ...
  // Admin Seed Configuration (apenas via variáveis de ambiente - sem defaults)
  // Para criar admin seed, definir todas as 3 variáveis no .env ou docker-compose
  SEED_DEFAULT_ADMIN: z.coerce.boolean().default(false), // Desactivado por defeito
  DEFAULT_ADMIN_EMAIL: z.string().email().optional(), // Sem default - obrigatório via env
  DEFAULT_ADMIN_PASSWORD: z.string().min(8).optional(), // Sem default - obrigatório via env
...
class ConfigService {
  ...
  private validateEnvironmentConfig(): void {
    ...
    // Validação de Admin Seed - garantir que credenciais estão definidas via env
    if (this._config.SEED_DEFAULT_ADMIN) {
      if (!this._config.DEFAULT_ADMIN_EMAIL || !this._config.DEFAULT_ADMIN_PASSWORD) {
        ...
        console.warn('   Admin seed será desactivado.');
        (this._config as any).SEED_DEFAULT_ADMIN = false;
      } else {
        console.log('✅ Admin seed configurado para:', this._config.DEFAULT_ADMIN_EMAIL);
      }
    }
